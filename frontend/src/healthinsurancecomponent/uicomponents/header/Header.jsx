...
function Header() {
    ...
    //         }, 1000);
    //     }
    //     return () => clearInterval(timer1); // Cleanup timer on unmount or when timer ends
    ...
    // }, [resendTimer,resendTimerEmail,resendTimerPhoneNumber]);
        ...
        if (name === "username") {
            setErrors({
                ...
                username: usernamePattern.test(value)
                    ? ""
                    : "username A-Z a-z or, no space but one middle space is allowd in sername",
            });
        }
    }
        ...
        if (name === "email") {
            setErrors({
                ...
                email: emailPattern.test(value)
                    ? ""
                    : "Enter Valid Email(all lower character and special character(abcde@gmail.com)",
            });
        }
    }
    ...
    const handleSendOtp = () => {
        ...
        axios.get(`http://192.168.1.18:9090/register/get/${values1.phoneNumber}`)
            .then((res) => {
                console.log(res.data);
    
                if (res.data === "found") {
                    // Generate OTP using the current minutes and seconds
                    // const date = new Date();
                    // const minutes = String(date.getMinutes()).padStart(2, '0');
                    // const seconds = String(date.getSeconds()).padStart(2, '0');
                    // const Otpxyz = minutes + seconds; // OTP
    
                    // setLoginOtp(Otpxyz); // Save OTP in state
                    // console.log("Generated OTP:", Otpxyz);
    
                    // // Prepare the message
                    // const phoneNumber = values1.phoneNumber; // Using `values1.phoneNumber`
                    // const message = `Dear customer, use this OTP ${Otpxyz} to sign up for your Quality Thought Next account. This OTP is valid for 15 minutes.`;
    ...
    //         axios.get("http://192.168.1.18:9090/register/get/" + values1.phoneNumber).then((res) => {
    //             console.log(res.data);
    //             if (res.data == "found") {
    //                 const date = new Date(); // Get the current date and time
    //                 const minutes = String(date.getMinutes()).padStart(2, '0'); // Pad minutes with leading zero
    ...
    //                     setIsOtpSent(true); // Show OTP boxes
    //                     isClicked(false);
    //                     // setOtp(["", "", "", ""]); // Reset OTP input
    //                     setResendTimer(30); // Reset timer
    //                     toast.info("OTP sent!");
    //             } else {
    //                 toast.error("phoneNumber is Not exists")
    //                 setIsOtpSent(false); // Show OTP boxes
    //                     isClicked(true);
    //                     setOtp(["", "", "", ""]); // Reset OTP input
    //                     setResendTimer(30); // Reset timer
    //                     toast.info("OTP sent!");
    ...
    const handleSendOtpEmail = () => {
        if (!values.email || errors.email) {
            ...
            axios.get("http://192.168.1.18:9090/register/call/" + values.email).then((res) => {
                console.log(res.data);
                if (res.data == "data Was not found with the Email") {
                    axios.get("http://192.168.1.18:9090/register/send-otp?email=" + values.email).then((res) => {
                        console.log(res.data)
    ...
    const handleSendOtpPhoneNumber = () => {
        if (!values.phoneNumber1 || errors.phoneNumber1) {
            ...
            axios.get("http://192.168.1.18:9090/register/get/" + values.phoneNumber1).then((res) => {
                console.log(res.data);
                if (res.data == "data Was not found with the MobileNo") {
                    ...
                    toast.error("phoneNumber is already exists")
    ...
    const handleOtpVerify = () => {
        ...
        if (otpString.length === 4) {
            ...
            toast.error("Please enter a valid 4-digit OTP."); // Display an error message
        }
    };
    ...
    const handleOtpVerifyRegisterMobile = () => {
        ...
        if (otpString.length === 4) {
            ...
            toast.error("Please enter a valid 4-digit OTP. For Mobile"); // Display an error message
        }
    };
    const handleOtpVerifyRegisteremail = () => {
        ...
        if (otpString.length === 4) {
            ...
            toast.error("Please enter a valid 4-digit OTP. For Email"); // Display an error message
        }
    };
    ...
    const handleRegister = (e) => {
        ...
        if (!username || !phoneNumber1 || !email || errors.username || errors.phoneNumber1 || errors.email) {
            ...
            if (isEmailValid && isMobileValid) {
                ...
                e.preventDefault();
                axios.post(
                    "http://192.168.1.18:9090/register/data", // API endpoint
                    data, // Request body
    ...
    return (
        <div>
            <nav className="navbar navbar-light bg-light">
                ...
                <div>
                    <button className="btn btn-primary mx-5" onClick={openModal}>
                        Login
                    </button>
                </div>
            </nav>
